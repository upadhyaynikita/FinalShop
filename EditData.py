...
def fetch_data():
    try:
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM DFS.DEMO.VW_STAGE WHERE ISSUE_NAME <> 'CORRECT RECORDS' AND ISSUE_NAME <> 'Duplicate records' UNION SELECT 'Duplicate records',     INVOICE_ID,     PURCHASE_ORDER ,     VENDOR_TAX_ID,     VENDOR_NAME,     VENDOR_ADDRESS,     VENDOR_ADDRESS_RECIPIENT,     CUSTOMER_NAME,     CUSTOMER_ID,     INVOICE_DATE,     SUB_TOTAL,     TOTAL_TAX,     INVOICE_TOTAL, NULL   FROM DFS.DEMO.STG_INVOICES WHERE (INVOICE_ID, INVOICE_DATE) IN( SELECT INVOICE_ID, INVOICE_DATE FROM DFS.DEMO.VW_INVOICES GROUP BY INVOICE_ID, INVOICE_DATE HAVING COUNT(*) > 1)")
